...
import bitoflife.chatterbean.ResourcesProvider;
...
public class ChatterBeanParser {
	...
	private Logger newLogger(String root, String dir) throws Exception {
		...
		return loggerClass.getConstructor(Writer.class).newInstance(
				new FileWriter(file));
	}

	private InputStream newResourceStream(String resource, String root,
			String path) throws Exception {
		if (root == null || path == null)
			throw new IllegalArgumentException(
					"Invalid path elements for retrieving " + resource
							+ ": root(" + root + "), path(" + path + ")");
	...
	public void parse(ChatterBean bot, String path)
			throws AliceBotParserException {

		if (path == null) {
			Properties properties = new Properties();
			try {
				properties.loadFromXML(ResourcesProvider.properties());
				
				botParser.parse(bot.getAliceBot(), ResourcesProvider.context(), ResourcesProvider.splitters(),
						ResourcesProvider.substitutions(),ResourcesProvider.aimls());
			} catch (Exception e) {
		...
		try {
			...
			InputStream context = newResourceStream("context", root,
			...
			botParser.parse(bot.getAliceBot(), context, splitters,
